...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  DBStat::UpdateWhileWriteFile(level);
  return s;
}
...
void DBImpl::BackgroundCompaction() {
  ...
  if (is_manual) {
    ...
    c = versions_->PickCompaction(options_);
  }
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  Log(options_.info_log,  "Compacting %d@%d + %d@%d files",
      ...
      compact->compaction->level() + 1);
  //statistics work
  int ll_num = compact->compaction->num_input_files(0);
  int hl_num = compact->compaction->num_input_files(1);
  int mylevel = compact->compaction->level();
  DBStat::UpdateWhileCompact(ll_num, hl_num, mylevel);
...
#endif
    DBStat::UpdateWhileIterKeyInCompact(input, drop);
    DBStat::UpdateStatIOWhileIterKey(drop, (ikey.type == kTypeDeletion), key.ToString());
    if (!drop) {
      ...
      //lhh add
      compact->builder->StatDelKeySize(ikey.type == kTypeDeletion, key.size());
  ...
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    ...
    DBStat::UpdateWhileWriteFile(compact->compaction->level() + 1);
  }
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      if (stats_[level].micros > 0 || files > 0) {
        ...
        value->append(buf);
        compact_io_mb += stats_[level].bytes_read / 1048576.0 + stats_[level].bytes_written / 1048576.0;
      }
    }
    snprintf(
           buf, sizeof(buf),
    ...
    *value = versions_->current()->DebugString();
    ...
    value->append(buf);
    return true;
  } else if (in == "diskspace") {
    std::ostringstream os;
    DBStat::PrintDiskSpace("this test", os);
    *value = os.str();
    return true;
  
  } else if (in == "levelsstnum") {
    std::ostringstream os;
    DBStat::PrintLevelSstNum(os);
    *value = os.str();
    return true;
  
  } else if (in == "clearstat") {
    DBStat::Clear();
    return true;
  
  } else if (in == "movenum") {
    std::ostringstream os;
    DBStat::PrintTrivialMoveNum(os);
    *value = os.str();
    return true;
  
  } else if (in == "deleteperc") {
    std::ostringstream os;
    DBStat::PrintCompactDeleteKeyPercent(os);
    *value = os.str();
    return true;
  
  } else if (in == "compactresult") {
    std::ostringstream os;
